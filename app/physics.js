...
export const update = (delta) => {
	...
	pills.forEach((pill, i) => {
		const wind = new CANNON.Vec3(
			pill.body.position.x / (WORLD_SIZE / 2) * Math.sin(time / 2000),
			pill.body.position.y / (WORLD_SIZE / 2) * Math.sin(time / 3000),
			pill.body.position.y / (WORLD_SIZE / 2) * Math.sin(time / 1000),
		);
		wind.normalize();
		const scale = Math.sin(time / 2000) * WIND_STRENGTH;
		pill.update(wind.scale(scale));
	});
